...
%#########################################################################%
%																		  %
%  Spring2016//CSCE699//Research project//Oladiran//Olaleye//CLID:ogo8842 %
%      Simulation of Energy Spectrum Sensing for Spectrum Awareness:      %
% A Novel Scheme for Minimizing the Effects of Noise Estimation Error and %
% Malicious Attacks in Energy-Detection-based Cooperative Spectrum Sensing%
%                   for Spectrum Situational Awareness                    %
%   						   02/15/2015                                 %
%						      Copyright(c)								  %
%#########################################################################%
%% This code plots:
...
%EIRP_primaryUser = 10*log10(1000); % dBm (unit)
EIRP_primaryUser = 50; % dBm EIRP (unit)
primarySignal_vector = ones(1,num_SecondaryUsers).* EIRP_primaryUser;
...
Fig4 = figure(4);
set(Fig4, 'Position', [350 150 520 500])
for num_model = 1:7
    plot(curvesX_caseC(num_model,:),curvesY_caseC(num_model,:),[graph_color(num_model),'-',graph_ID(num_model)]), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
xlabel('Fused Probability of False Alarm');
ylabel('Fused Probability of Detection');
legend('\Delta_N = -3','\Delta_N = -2','\Delta_N = -1',...
      '{\itP}^{avg}_N = -65.0 dBm','\Delta_N = 1', '\Delta_N = 2','\Delta_N = 3');
...
%% Plotting the ROC Curve
Fig5 = figure(5);
set(Fig5, 'Position', [350 150 520 500])
for num_model = 1:7    
    plot(curvesX_caseN(num_model,:),curvesY_caseN(num_model,:),[graph_color(num_model),'-',graph_ID(num_model)]), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
%% Plotting the ROC Curve
Fig6 = figure(6);
set(Fig6, 'Position', [350 150 520 500])
for num_model = [4 1 7]
    plot(curvesX_caseT(num_model,:),curvesY_caseT(num_model,:),[graph_color(num_model),'-',graph_ID(num_model)]), axis equal
    grid on
    hold on
end
box on
...
xlabel('Fused Probability of False Alarm');
ylabel('Fused Probability of Detection');
legend('\Delta_N=0 dBm, {\itP}^{avg}_N = -65.0 dBm','-5\leq\Delta_N\leq+5 dBm, Conventional Method','-5\leq\Delta_N\leq+5 dBm, Proposed Scheme');
...
%% Plotting the ROC Curve
Fig7 = figure(7);
set(Fig7, 'Position', [350 150 520 500])
for num_model = 1:8    
    plot(curvesX_caseP(num_model,:),curvesY_caseP(num_model,:),[graph_color2(num_model),'-',graph_ID(num_model)]), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
xlabel('Fused Probability of False Alarm');
ylabel('Fused Probability of Detection');
legend('\Delta_S = -3','\Delta_S = -2','\Delta_S = -1',...
      'EIRP = 50.0 dBm','\Delta_S = 1', '\Delta_S = 2','\Delta_S = 3','\Delta_S = {\itP}^{meas}_S');
...
%% Plotting the ROC Curve
Fig8 = figure(8);
set(Fig8, 'Position', [350 150 520 500])
for num_model = 1:8    
    plot(curvesX_caseR(num_model,:),curvesY_caseR(num_model,:),[graph_color2(num_model),'-',graph_ID(num_model)]), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
%% Plotting the ROC Curve
Fig9 = figure(9);
set(Fig9, 'Position', [350 100 580 580])
for num_plot = 1:num_model
    plot(curvesX_caseD(num_plot,:),curvesY_caseD(num_plot,:),[graph_color3(num_plot),'-',graph_ID2(num_plot)],'MarkerSize',graphMarker_size(num_plot)), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
xlabel('Fused Probability of False Alarm');
ylabel('Fused Probability of Detection');
legend('EIRP = 50.0 dBm, {\itN}^{honest}_{SU} = 10, {\itN}^{maliciuos}_{SU} = 0',...
       '\Delta_S = -2, {\itN}^{honest}_{SU} = 5, {\itN}^{maliciuos}_{SU} = 5',...
       '\Delta_S = -2, {\itN}^{honest}_{SU} = 0, {\itN}^{maliciuos}_{SU} = 10',...       
       '\Delta_S = {\itP}^{meas}_S, {\itN}^{honest}_{SU} = 5, {\itN}^{maliciuos}_{SU} = 5',...
       '\Delta_S = {\itP}^{meas}_S, {\itN}^{honest}_{SU} = 0, {\itN}^{maliciuos}_{SU} = 10');
...
%% Plotting the ROC Curve
Fig10 = figure(10);
set(Fig10, 'Position', [350 100 580 580])
for num_plot = 1:num_model
    plot(curvesX_caseE(num_plot,:),curvesY_caseE(num_plot,:),[graph_color3(num_plot),'-',graph_ID2(num_plot)],'MarkerSize',graphMarker_size(num_plot)), axis equal
    hold on
end
grid on
%title('ROC Curve for');
...
Fig11 = figure(11);
set(Fig11, 'Position', [350 150 520 500])
for num_model = [4 1 7]
    plot(curvesX_caseV(num_model,:),curvesY_caseV(num_model,:),[graph_color2(num_model),'-',graph_ID(num_model)]), axis equal
    grid on
    hold on
end
box on
...
xlabel('Fused Probability of False Alarm');
ylabel('Fused Probability of Detection');
legend('\Delta_S=0 dBm, EIRP = 50.0 dBm.','-5\leq\Delta_S\leq+5 dBm, Conventional Method','-5\leq\Delta_S\leq+5 dBm, Proposed Scheme');
